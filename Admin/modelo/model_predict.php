...
class model_predict extends modelo_conexion
{
    ...
    function grafica_cajas_2018_2020()
    {
        try {
            ...
            $sql = 'SELECT * FROM temp_cajas_año ORDER BY id DESC';
            $query = $c->prepare($sql);
            $query->execute();
    ...
    function grafica_enfunde_2018_2020()
    {
        try {
            $c = modelo_conexion::conexionPDO();
            $sql = 'SELECT * FROM temp_enfunce_año ORDER BY id DESC';
            $query = $c->prepare($sql);
            $query->execute();
            $result = $query->fetchAll(PDO::FETCH_BOTH);
            $arreglo = array();
    ...
    function grafica_recobro_2018_2020()
    {
        try {
            $c = modelo_conexion::conexionPDO();
            $sql = 'SELECT * FROM temp_recobro_años ORDER BY id DESC';
            $query = $c->prepare($sql);
            $query->execute();
            $result = $query->fetchAll(PDO::FETCH_BOTH);
            $arreglo = array();
    ...
    function grafica_produccion_2018_2020()
    {
        try {
            $c = modelo_conexion::conexionPDO();
            $sql = 'SELECT * FROM temp_produccion_año ORDER BY id DESC';
            $query = $c->prepare($sql);
            $query->execute();
            $result = $query->fetchAll(PDO::FETCH_BOTH);
            $arreglo = array();
