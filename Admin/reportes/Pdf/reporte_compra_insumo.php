...
while ($row = $result->fetch_assoc()) {
  ...
  if($row['tipo_comprobante'] == 'Notacompra'){
    ...
    $sqldetalle = 'SELECT
                detalle_compra_insumo.id_compra,
                insumo.nombre,
                ...
                INNER JOIN insumo ON detalle_compra_insumo.id_insumo = insumo.id
                INNER JOIN tipo_insumo ON insumo.tipo = tipo_insumo.id 
                WHERE
                detalle_compra_insumo.id_compra = "' . $_GET["id"] . '"';
    ...
    while ($rowmedi = $resultmedi->fetch_assoc()) {
        ...
        $html .= '<tr>
                <td class="desc"> ' . $rowmedi['nombre'] . ' ' . $rowmedi['tipo_insumo'] . '  </td>
