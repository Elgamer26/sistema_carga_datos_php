...
$html = '<!DOCTYPE html>
    <html lang="en">
      <head>
        <meta charset="utf-8">
        <title>Informe de encinte</title>
      ...
      <body>  
            ...
            <table style="border-collapse;" border="1">
                <thead>
                        <tr>
                            ...
                            <h1  style="color: black;"> Encinte </h1>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style="color: black; ">SEMANA</th> 
        ...
        FROM
        tipo_cinta
        INNER JOIN encinte ON tipo_cinta.id = encinte.cinta_id
        INNER JOIN produccion ON encinte.produccion_id = produccion.id
        INNER JOIN lote ON produccion.lote_id = lote.id  
        WHERE
        encinte.fecha BETWEEN ' . $_GET["f_i"] . ' AND ' . $_GET["f_f"] . '
        ORDER BY
        encinte.id ASC ';
...
while ($rowmedi = $resultmedi->fetch_assoc()) {
    ...
    $html .= '<tr>
                ...
                <td class="desc"> ' . $rowmedi['semana'] . ' </td>  
                <td class="desc"> ' . $rowmedi['fecha'] . ' </td>
                <td class="desc"> ' . $rowmedi['detalle'] . ' </td>  
